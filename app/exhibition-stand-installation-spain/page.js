        ...
        <div className="container heroGridInstall">
          <div>
            ...
            <div className="pillRow">
              {["IFEMA Madrid", "Fira Barcelona", "White-label / NDA", lang === "es" ? "Cobertura nacional" : "Nationwide"].map((p) => (
          ...
          <div className="installBox">
            <div className="installBoxHead">
              <div className="installBoxTitle">{lang === "es" ? "Qué incluye" : "What's included"}</div>
            </div>
            <ul className="installList">
              {page.services.cards.map((c) => <li key={c.title}><strong>{c.title}</strong> — {c.text}</li>)}
            </ul>
            <div className="installBoxCtas">
              <Link className="btn btnSmall" href="/contact">{lang === "es" ? "Hablar del stand" : "Talk about your stand"}</Link>
            </div>
          </div>
        </div>
      </section>
